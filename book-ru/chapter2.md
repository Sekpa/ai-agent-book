...
И это не только техническая проблема, но прежде всего **проблема организационная**. В большинстве команд ключевое знание неявное: архитектурные решения помнят лишь старожилы, бизнес-правила передаются из уст в уста, а важная фоновая информация заперта в личных переписках. Если сама команда — чёрная дыра для информации, то и лучший AI-агент бессилен.
...
Разобравшись со структурой JSON, давайте свяжем весь этот процесс взаимодействия воедино с помощью кода на Python. Ниже — простейшая реализация агента, ядро которой представляет собой цикл while: Эта глава намеренно сохраняет полный цикл работы с API как справочник по протоколу; в остальных главах механизмы поясняются скелетным кодом в стиле Python.
...
У последующих техник много названий, но перед каждым запросом всё сводится к одному решению о построении контекста. Ниже псевдокод в стиле Python сохраняет минимальный скелет этого решения; он дополняет полный цикл работы с API выше, делая упор на компоновку контекста, и не заменяет деталей протокола вроде ролей сообщений и `tool_call_id`.
...
Системный промпт и определения ключевых инструментов держите как можно более стабильными; старые результаты инструментов сжимайте только пакетно, по мере приближения к бюджету; текущее состояние помещайте в хвост траектории, чтобы модели не приходилось заново выводить его из длинной истории.
...
Важно учитывать, что семейства моделей по-разному обращаются с историей цепочки рассуждений, а сами политики быстро меняются. Во времена DeepSeek R1 официальная практика предписывала **удалять все прежние рассуждения**: в многораундовом диалоге возвращать только `content`, но не `reasoning_content`, поскольку при обучении R1 прежние CoT никогда не встречались во входе; их возврат создавал ввод вне обучающего распределения, который мог мешать выводу, а удаление заодно экономило немало токенов. Но для сценариев с агентами у такой политики есть недостаток: промежуточные рассуждения несут важное состояние — «почему вызван этот инструмент, какие гипотезы исключены»; после удаления модель рассуждает с нуля в каждом раунде, легко повторяет ошибки и теряет долгосрочный план. Поэтому в V4 DeepSeek **полностью развернула** политику: если запрос содержит параметр `tools`, то `reasoning_content` каждого сообщения assistant между двумя сообщениями user — даже того, в котором инструмент фактически не вызывался, — нужно возвращать дословно, иначе API сразу выдаёт ошибку 400; в обычном диалоге без `tools` прежние рассуждения по-прежнему игнорируются. Агент всегда передаёт `tools`, поэтому обойти это требование не получится. Такой же протокол приняли Kimi K2, GLM-5 и другие модели. Claude тоже требует возвращать thinking block без изменений вместе с подписью; более новые модели Claude могут использовать прежние thinking и за пределами одного пользовательского раунда, но подпись привязывает каждый thinking block к префиксу, при котором он был создан, и стоит префиксу измениться, как блок становится недействительным (подробнее см. раздел «Кэширование как архитектурное ограничение» этой главы). Поэтому перед использованием нужно сверяться с актуальной документацией модели. В многораундовом диалоге эти различия решают лишь то, экономятся ли токены, но как только недоигранную траекторию приходится передавать модели другого поставщика, они превращаются в настоящие ошибки интерфейса — см. эксперимент 5-1 в главе 5.

**Во-вторых, это объясняет, почему KV Cache так чувствителен к префиксу**. Chat Template преобразует системное сообщение и определения инструментов в фиксированную последовательность токенов, размещённую в самом начале. Пары ключ-значение (Key-Value pairs) этих токенов после кэширования можно переиспользовать между запросами. Но если токен в префиксе изменится — даже если в системном промпте появился лишний пробел, — кэш нельзя будет переиспользовать начиная с первого отличающегося токена. Рис. 2-10 показывает именно такое переиспользование префикса между запросами: по различению из раздела «KV Cache и Prompt Cache: два уровня кэширования» ниже, оно происходит на уровне Prompt Cache, а переиспользуется при этом KV Cache префикса.
...
![Рис. 2-10 Prompt Cache: переиспользование префиксного KV Cache между запросами](images/fig2-10.svg)

**Разберём KV Cache на простом примере**. Пусть в контексте 4 токена [A, B, C, D], и модель готовится сгенерировать пятый токен E. Ключевая операция внимания: вектор запроса (Query) на этом шаге берётся у последнего известного токена D и скалярно умножается на векторы ключей (Key) четырёх токенов A, B, C, D, чтобы вычислить степень соответствия (интуитивный смысл скалярного произведения см. в эксперименте 2-2), а затем на основе этих значений соответствия вычисляется взвешенная сумма векторов значений (Value) тех же четырёх токенов — так получается выходное представление позиции D, по которому модель и предсказывает следующий токен E. Собственные Q, K, V токена E будут вычислены только после того, как E будет сэмплирован и подан обратно в модель.

Без использования KV Cache при генерации каждого нового токена приходится заново прогонять через модель весь префикс: при генерации E нужно вычислить 4 пары K, V для A, B, C, D, при генерации шестого токена — уже 5 пар, вместе с E... когда префикс дорастает до N токенов, нужно вычислить N пар, и накопленный объём вычислений пропорционален N².

При использовании KV Cache векторы K, V каждого токена вычисляются один раз — когда он впервые попадает в контекст, — и дальше остаются в кэше. На шаге генерации E 4 пары для A, B, C, D уже лежат в кэше, их достаточно прочитать, чтобы выполнить вычисление внимания; и только после того, как E будет сэмплирован и подан обратно в модель, вычисляются собственные K, V токена E и добавляются в кэш, который вырастает до 5 пар — для генерации шестого токена. Стоит отметить: KV Cache избавляет от повторного пересчёта проекций K, V для исторических токенов, так что на каждом шаге декодирования не нужно пересчитывать весь префикс заново; но вычисление внимания для каждого нового токена всё равно требует обхода всех закэшированных K, V, и объём вычислений растёт линейно с длиной контекста — именно поэтому декодирование длинного контекста становится всё медленнее, а видеопамять и пропускная способность для KV Cache становятся узким местом вывода.
...
**Изменение префикса стоит не только денег — можно потерять и рассуждения**. Всё сказанное выше касалось кэша: префикс изменился — запрос стал медленнее и дороже, но результат по-прежнему верен. Механизм «сохранённых рассуждений» (preserved thinking)[^ch2-preserved-thinking], который Anthropic ввела начиная с Claude Fable 5.1 и Claude Opus 5.5, придаёт стабильности префикса ещё один смысл. Изначально он задуман для защиты от дистилляции: сервер с помощью подписи привязывает каждый thinking block к префиксу, при котором тот был создан, — то есть к system и tools верхнего уровня и ко всем предшествующим сообщениям. Если в последующем запросе этот префикс хоть в чём-то изменится, недействительными станут и этот thinking block, и все thinking после него. Новые аккаунты по умолчанию сразу получают ошибку 400; можно вместо этого включить молчаливое отбрасывание недействительных thinking — запрос пройдёт успешно, но в этом раунде модель не сможет опереться на прежние рассуждения и будет думать с нуля.

Защита от дистилляции и кэширование — разные вещи, но инженерная дисциплина из них следует одна и та же. Официальная документация говорит прямо: thinking делают недействительными ровно те изменения, из-за которых кэш пересчитывается с нуля. И опасные приёмы всё те же, знакомые: пересобирать системный промпт в каждом раунде (например, вписывая текущее время или переключатель режима), заново рендерить сведения об окружении в первом пользовательском сообщении, добавлять или убирать `tools` посреди сессии, укорачивать старые результаты инструментов на месте, вставлять в старые сообщения напоминания, а потом удалять их. И решение то же — дописывать изменения в конец: новые инструкции подавать отдельным system-сообщением, добавленным в диалог, изменения окружения записывать в последний раунд, а добавление и удаление инструментов объявлять специальным блоком сообщения, не трогая массив `tools`. Сами thinking block тоже нельзя обрезать как угодно: удалять можно с начала, с конца или все сразу, но нельзя убрать один из середины, оставив последующие, а удалённые нельзя возвращать обратно. Если посреди сессии сменить модель, а новая не умеет читать thinking исходной, API молча отбросит эти блоки, и несколько раундов после переключения пройдут без прежних рассуждений.

Ключевой вывод из этих проектных решений таков: **при проектировании архитектуры агента экономика кэширования — не постфактум-оптимизация, а изначальное ограничение**. Чем раньше его учесть в архитектуре, тем меньше будут последующие инженерные издержки. Когда к префиксу привязана и цепочка рассуждений, принцип «только дописывать, не переписывать» превращается из совета по производительности в требование корректности.

[^ch2-preserved-thinking]: Anthropic, "Preserved thinking", документация Claude API. https://platform.claude.com/docs/en/build-with-claude/preserved-thinking

### KV Cache необязательно одноразовый: редактируемые и компонуемые «заметки»

(Дальше идёт дополнительное чтение из области исследовательского фронтира — материал для «глубокого погружения»; при первом чтении его можно пропустить и сразу перейти к следующему подразделу, это не помешает пониманию дальнейшего содержания главы. А вот три практических вывода, изложенных выше, — это фундамент, который необходимо усвоить.)
...
### Переход: от механики кэширования к проектированию содержимого контекста
...
Например, один системный промпт может использовать оба формата сразу:
...
## Работа с файлами
<file_operation>
- Перед чтением файла обязательно проверить, существует ли путь
- Перед записью в файл обязательно сделать резервную копию
</file_operation>

## Сетевые запросы
<network_request>
- Тайм-аут — 30 секунд
- После сбоя повторять не более 3 раз
</network_request>
```

- **Роль Markdown**: заголовки `#` и `##` позволяют человеку с одного взгляда увидеть иерархию — промпт остаётся читаемым.
- **Роль XML**: теги `<file_operation>` и `<network_request>` сообщают модели, что «этот блок про работу с файлами», а «этот блок про сетевые запросы»; семантика точная, и модель обрабатывает такой промпт аккуратнее.

Вместе они дают промпт, который человеку читать понятно, а модели — понимать точно.
...
**Второй уровень (основной процесс)**: когда агент определяет, что задаче нужен конкретный Skill, среда выполнения только тогда загружает полный `SKILL.md`. Загрузку запускают двумя способами: когда пользователь вводит явную слеш-команду (например, `/pptx`), клиент перехватывает и разворачивает её локально, так что модели не нужно сначала делать вызов инструмента; когда же модель, прочитав каталог метаданных, сама решает, что Skill нужен, она вызывает специальный инструмент Skill — это лишний цикл ReAct. Обе дорожки приводят в одно и то же место: Claude Code добавляет тело Skill как user message в точке вызова, а tool result на пути, который начала модель, — лишь заглушка с сообщением о запуске Skill, тела в ней нет[^ch2-cc-skill-inject]. В средах без специального инструмента активации модель читает `SKILL.md` обычным инструментом чтения файлов, и тогда тело попадает в контекст как tool result. Например, PPTX Skill[^ch2-4] содержит основной процесс работы с PowerPoint: извлечение текста через markitdown, распаковку PPTX для доступа к исходной XML-структуре и соглашения о путях к ключевым файлам.

[^ch2-4]: Anthropic, "PPTX Skill", 2025. https://github.com/anthropics/skills/
[^ch2-cc-skill-inject]: Claude Code Docs, [«How Claude Code uses prompt caching»](https://code.claude.com/docs/en/prompt-caching), раздел «Invoking skills and commands»: «Skills and commands inject their instructions as user messages at the point of invocation». О различии между явным запуском и запуском по решению модели см. Agent Skills, [«How to add skills support to your agent»](https://agentskills.io/client-implementation/adding-skills-support), раздел «User-explicit activation»: слеш-команду перехватывает Harness и сам вставляет содержимое, так что модели не нужно выполнять действие активации.
...
- **Claude Code концептуально**: предоставляет небольшой каталог как контекст среды выполнения и добавляет полные инструкции в точке вызова Skill. «Системный промпт» может описывать логически стабильный слой инструкций, но это не утверждение, что каждый клиент использует API-роль `system`. Рис. 2-12 показывает случай, начатый моделью: в траектории виден полный цикл — tool_use `Skill(skill: "pptx")`, заглушка tool_result и затем тело, добавленное отдельным user-сообщением. Если пользователь вводит `/pptx` напрямую, клиент разворачивает команду локально, поэтому этой пары служебных сообщений в траектории нет и остаётся только последнее user-сообщение.
...
Agent Harness быстро развиваются, поэтому конкретное представление может измениться. Стабильный принцип — **небольшой обнаруживаемый каталог и полное содержимое, загружаемое по требованию**. Две картинки ниже показывают позицию Skills в trajectory и эволюцию KV Cache. Чтобы наглядно почувствовать эффект такого устройства, две следующие иллюстрации с двух разных точек зрения прослеживают, где Skills располагаются в траектории и как эволюционирует KV Cache.
...
Побочная информация о событии обычно добавляется вместе с самим событием; планирование задачи и состояние среды, напротив, непрерывно обновляются по мере продвижения работы. То, как эта динамическая информация записывается в историю диалога, напрямую связано со стоимостью KV Cache, и ниже мы разберём это вместе с конкретной структурой сообщений.
...
**Во-вторых, повышение качества мышления — обобщённое знание модель использует лучше, чем сырую форму.** Этот мотив лежит глубже, и его легче упустить. Даже если окно контекста достаточно велико, сваливать в контекст всю сырую информацию — не лучший выбор: сырые результаты десятка раундов поиска рассеяны по всему контексту, поэтому при каждом решении модели приходится снова и снова разыскивать нужные фрагменты среди десятков тысяч токенов, внимание рассеивается, а ключевая информация легко теряется. Если же одним вызовом LLM сначала свести накопленное в структурированную сводку — «известно на данный момент: A — это…, B — это…, не хватает сведений о C», — то дальнейшее рассуждение сможет пользоваться этим очищенным представлением напрямую. В следующем разделе объясняется стоящий за этим механизм.
...
Когда вы спрашиваете «сколько чёрных кошек и сколько белых?», модели без включённой цепочки рассуждений трудно ответить правильно сразу: **поиск** («какая кошка в клетке 37?») — сильная сторона внимания, а вот **статистическое обобщение** («сколько всего чёрных кошек?») требует обойти все записи и удерживать состояние счётчика, что по сути является размышлением, а не извлечением. Включённая цепочка рассуждений, конечно, позволит сосчитать верно, но при каждом вопросе считать придётся заново; в агентских сценариях такая статистика часто используется многократно, и накопленная стоимость размышлений оказывается высокой. Если же заранее один раз подвести итог и прямо записать в контекст «текущая статистика: 90 чёрных кошек, 10 белых», модель тут же извлечёт этот вывод. **В этом и состоит вторая ценность сжатия: превратить выводы, которые даются только размышлением, в знание, доступное прямому извлечению.**
...
Если модель привязывает thinking к префиксу (см. выше раздел «Кэширование как архитектурное ограничение»), сжатие обходится дороже, чем просто потеря кэша. Распространённый приём «резюмировать старые раунды, а последние несколько оставить как есть» здесь даёт сбой: thinking в сохранённых раундах создавались, когда перед ними стояла исходная история, а не резюме, — префикс изменился, и все они становятся недействительными; то же касается укорачивания старых результатов инструментов на месте. Anthropic рекомендует один из двух подходов: либо сжать весь диалог в одно сообщение-резюме и больше не возвращать никаких старых thinking, чтобы модель заново рассуждала, отталкиваясь от резюме; либо поручить сжатие или очистку контекста серверу — такие серверные изменения не считаются модификацией префикса.
...
Основная линия контекстной инженерии — явное управление информацией: структура сообщений API задаёт каркас; стабильный префикс повышает попадания в KV Cache; промпт, Skills и строка состояния несут соответственно правила, знание по требованию и текущее состояние; а сжатие повышает информационную плотность истории, сохраняя при этом решения, ограничения, отказы и источники.
