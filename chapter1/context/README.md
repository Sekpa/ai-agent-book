...
把 Agent 的一轮执行写成“读消息 → 选择动作 → 执行工具 → 读工具结果”。历史消息帮助它知道任务进展，工具定义帮助它选择动作，工具结果提供刚刚取得的事实。实验中的消融开关分别移除这些输入，因此应先读懂完整模式的轨迹，再观察移除信息以后哪里发生变化。推理文本与外部执行结果来源不同；不能把模型自己的解释当作工具已经执行的证据。
...
工具结果和历史消息可能帮助模型减少猜测和重复操作，但影响取决于任务与模型。
简单任务即使缺少某类信息也可能做对；某次运行更快，也不足以证明一种配置总是更高效。
尤其是“去掉推理内容会退化”并非本仓库每次实测都能复现。要讨论稳定差异，应在多个任务上重复运行，
并查阅 [实验记录](../EXPERIMENT_LEDGER.md) 中的实际证据。
...
先区分“程序给出了最终响应”和“任务做对了”。例如，模型可能没有查询汇率，便用记忆中的数字算出答案；
程序虽然结束了，但这些数字未必有依据。阅读结果时，结合任务要求检查数字来源和工具记录。

比较不同上下文模式时，保持任务、模型和运行条件一致。重点看：哪些工具被调用、结果是否被模型看到、
有没有重复操作、答案是否符合任务要求。运行时间和调用次数帮助解释成本，不能单独代表答案质量。
...
Tool results and history can reduce guessing and repeated work, but their effect depends on the task and model.
A simple task may still succeed when a component is missing. One faster run does not establish that a configuration
is consistently more efficient. In particular, removing reasoning content has not caused degradation in every
recorded run. Repeat the comparison across tasks and consult the [experiment record](../EXPERIMENT_LEDGER.md)
before drawing a general conclusion. A terminal response alone does not establish task success: check the evidence
behind the answer and whether the required tools were actually used.
