...
> 適応する前にまず診断します。アーキテクチャ文書と PRD に照らして新しい形式が想定どおりかを判断します（実験 5-11 の考え方）。バージョン管理の記録を突き合わせ、変化が正当なコードコミットに対応するのか、出所のないドリフトなのかを確認します。τ-bench の log_mismatch になぞらえ、適応を選ぶ場合でも警告を記録し、黙って互換させるのではなく自動で issue を立てます。不確かなときは人間参加型（human-in-the-loop）の確認を経ます。原則：適応と報告を並行させ、適応が異常のシグナルを飲み込まないようにします。
...
**7. (★★★) 履歴再生において、ある探索方策がすべての旧い発見木で最高得点を取りながら、次のラウンドのオンライン探索では退化した。どのようなサポート領域の偏り、ランダム性、評価の過学習がこの結果を生みうるか。再生世界をどう分割し、リリースゲートをどう設計するか。**

> 旧い発見木が覆うのは旧方策が訪れた状態と分岐だけであり、未探索領域での性能は証明できません。オンラインのタスク、ツール、環境も変化しえます。乱数シード、モデルのサンプリング、ツールのノイズで順位が変わり、同じ木で選抜を繰り返すと評価への過学習が起こります。タスク群、時期、独立した木を単位に開発・検証・封印したテスト用の世界を分け、同じ木と派生軌跡を集合間で共有しません。モデルとツールの版、計算予算を固定し、複数シードで基準方策との対応のある比較を行い、改善幅、不確実性、コスト、違反率を報告します。品質・安全・コストの基準を事前に定め、オフライン合格後に小規模なオンライン対照実験で改善とガードレールの非退化を確認してから、ロールバック条件を設けて段階的に公開します。
