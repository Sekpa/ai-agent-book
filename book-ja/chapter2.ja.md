...
そしてこれは技術的な問題であるだけでなく、それ以上に**組織の問題**でもあります。多くのチームでは、鍵となる知識は暗黙のうちにあります。アーキテクチャ上の決定は古参の社員しか覚えておらず、業務ルールは口伝えで受け継がれ、重要な背景情報は個人間のチャット履歴の中に閉じ込められています。チームそのものが情報のブラックホールであれば、どれほど優れた AI Agent でも打つ手がありません。
...
JSON 構造を理解したところで、上記のインタラクションの過程を Python コードでつなげてみましょう。以下は最小限の Agent の実装です。核心となるのは 1 つの while ループだけです。 本章はこの完全な API ループをあえてプロトコルの参照として残します。他の章では Python 風のスケルトンコードで仕組みを説明します。
...
後続の技術には多くの名前がありますが、各リクエストの直前まで落とし込めば、実のところ一度のコンテキスト構築の判断にすぎません。以下では Python 風の擬似コードでその判断の最小の骨組みを残します。これは先の完全な API ループを補うものであり、コンテキストのレイアウトを強調するもので、メッセージのロールや `tool_call_id` といったプロトコルの細部を置き換えるものではありません。
...
システムプロンプトと中核的なツール定義はできるかぎり安定に保ちます。古いツール出力は予算に近づいたときにまとめて圧縮します。現在の状態は軌跡の末尾に置き、モデルが長い履歴から再び導き直さなくて済むようにします。
...
注意すべきは、過去の思考連鎖をどう扱うかはモデルファミリーごとに大きく異なり、その方針自体も急速に変化していることです。DeepSeek R1 時代の公式方針は、**過去の思考をすべて取り除く**ことでした。複数ラウンドの対話では `content` だけを返し、`reasoning_content` は返しません。R1 の学習入力に過去の CoT が現れたことがなく、戻せば分布外入力となって出力を妨げる可能性があり、同時に大量の token も節約できたからです。しかし Agent の場面では、この方針に欠点があります。中間思考は「なぜこのツールを呼んだか、どの仮説を除外したか」といった重要な状態を担っており、取り除くとモデルは毎ラウンドゼロから推論し、誤りを繰り返したり長期計画を失ったりしやすくなります。そのため DeepSeek は V4 で方針を**完全に反転**しました。リクエストが `tools` パラメータを伴う限り、2 つの user メッセージの間にあるすべての assistant メッセージ（そのラウンドで実際にはツールを呼ばなかったものも含む）の `reasoning_content` をそのまま返す必要があり、返さなければ API は 400 エラーを返します。`tools` を伴わない純粋な対話では、過去の思考は従来どおり無視されます。Agent は必ず `tools` を伴うため、この必須ルールからは逃れられません。Kimi K2、GLM-5 なども同じプロトコルを採用しています。Claude も同様に、thinking block を署名ごとそのまま返すよう求めます。新しい Claude モデルはユーザーのターンをまたいで過去の thinking を使うこともできますが、署名が各 thinking block をそれが生成されたときのプレフィックスに結びつけているため、プレフィックスが変わるとその block は無効になります（本章の「アーキテクチャ制約としてのキャッシュ」を参照）。したがって、利用前に各モデルの最新ドキュメントを確認してください。これらの違いは複数ターンの対話ではトークンを節約できるかどうかにしか関わりませんが、走りきっていない軌跡を別ベンダーのモデルに渡して続けさせようとした途端、実際の API エラーになります。詳しくは第 5 章の実験 5-1 を参照してください。

**第二に、なぜ KV Cache がプレフィックスにこれほど敏感かを説明します**。Chat Template は system メッセージとツール定義を固定の token シーケンスに変換して最前に置きます。これらの token のキー・バリューのペア（Key-Value pairs）はキャッシュされると、リクエストをまたいで再利用できます。しかしプレフィックス内の token が変化すると——たとえシステムプロンプトに空白が 1 つ増えただけでも——最初に異なる token 以降のキャッシュは再利用できません。 図2-10 が示しているのはまさにこのリクエストをまたいだプレフィックスの再利用です。後述の「KV Cache と Prompt Cache：2 つの層のキャッシュ」の区別で言えば、これは Prompt Cache の層で起きており、再利用されるのはプレフィックスの KV Cache です。
...
![図2-10 Prompt Cache：プレフィックスの KV Cache をリクエスト間で再利用](images/fig2-10.svg)

**シンプルな例で KV Cache を理解する**。コンテキストに 4 つの token [A, B, C, D] があり、モデルが今まさに 5 つ目の token E を生成しようとしているとします。アテンションの核となる操作はこうです。このステップのクエリベクトル（Query）は最後の既知 token である D のもので、これが A、B、C、D という 4 つの token のキーベクトル（Key）と内積を取ってマッチ度を計算し（内積の直感的な意味は実験 2-2 を参照）、さらにマッチ度にもとづいてこの 4 つの token のバリューベクトル（Value）を加重和して、D の位置の出力表現を得ます。モデルはこれを使って次の token E を予測します。E 自身の Q、K、V が計算されるのは、E がサンプリングされてモデルに戻された後です。

KV Cache を使わないとき、新しい token を 1 つ生成するたびにプレフィックス全体を最初から順伝播させ直します。E を生成するときは A、B、C、D の 4 組の K、V を計算し、6 つ目の token を生成するときは E も含めて 5 組……プレフィックスが N 個の token になれば N 組を計算し、累計の計算量は N² に比例します。

KV Cache を使うとき、各 token の K、V はそれがコンテキストに初めて入ったときに一度だけ計算され、その後はキャッシュに残ります。E を生成するステップでは、A、B、C、D の 4 組の K、V はすでにキャッシュにあるので、そのまま読み出してアテンション計算を完了できます。E がサンプリングされてモデルに戻された後にはじめて E 自身の K、V が計算されてキャッシュに追加され、キャッシュは 6 つ目の token を生成するための 5 組になります。注意すべきは、KV Cache が省くのは過去の token の K、V 射影の再計算であり、各デコードステップでプレフィックス全体を計算し直さずに済ませられる、ということです。しかし各新しい token のアテンション計算は依然としてキャッシュされたすべての K、V を走査する必要があり、計算量はコンテキスト長とともに線形に増大します——これこそが長いコンテキストのデコードがますます遅くなり、KV Cache の VRAM と帯域が推論のボトルネックになる理由です。
...
**プレフィックスを変えると、コストが増えるだけでなく推論も失われうる**。ここまでの話はすべてキャッシュに関するものでした。プレフィックスが変わればリクエストは遅く、高くなりますが、結果は正しいままです。Anthropic が Claude Fable 5.1 と Claude Opus 5.5 から導入した「保持思考」（preserved thinking）の仕組み[^ch2-preserved-thinking]は、プレフィックスの安定にもう一つの意味を加えました。この仕組みの本来の目的は蒸留対策です。サーバーは署名によって各 thinking block を、それが生成されたときのプレフィックス、つまりトップレベルの system と tools、およびそれより前のすべてのメッセージに結びつけます。以後のリクエストでこのプレフィックスに少しでも変更があると、その thinking block とそれ以降のすべての thinking が無効になります。新規登録のアカウントでは、デフォルトで 400 エラーがそのまま返されます。無効になった thinking を黙って破棄する設定に切り替えることもでき、その場合リクエストは通常どおり成功しますが、モデルはこのターンでそれまでの推論を使えず、一から考え直すことになります。

蒸留対策とキャッシュは別の話ですが、そこから導かれる工学上の規律はまったく同じです。公式ドキュメントも率直に述べているとおり、thinking を無効にする変更は、まさにキャッシュを最初から計算し直させる変更です。つまずきやすい書き方もおなじみのものばかりです。毎ラウンドシステムプロンプトを組み直す（現在時刻やモードの切り替えを書き込むなど）、最初のユーザーメッセージ内の環境情報を毎ラウンド描画し直す、途中で `tools` を増減する、古いツール結果をその場で切り詰める、古いメッセージにリマインダーを差し込んでから消す、といったものです。対処法も同じく、変化を末尾に追加することです。新しい指示は会話に追加する system メッセージで伝え、環境の変化は最新のラウンドに書き込み、ツールの増減は `tools` 配列を書き換えずに専用のメッセージブロックで宣言します。thinking block 自体も勝手に刈り込んではいけません。先頭から削る、末尾から削る、すべて削ることはできますが、途中の 1 つを削って後ろを残すことはできず、削ったものを戻すこともできません。途中でモデルを切り替えたとき、新しいモデルが元のモデルの thinking を読めなければ、API はそれらの block を黙って破棄し、切り替え後の数ターンはそれまでの推論なしで進むことになります。

これらの設計上の選択から得られる核心は、**Agent アーキテクチャを設計するとき、キャッシュの経済性は事後の最適化ではなく、先に置くべき制約である**ということです。この制約を早くアーキテクチャ設計に組み込むほど、その後の工学的コストは小さくなります。推論の連鎖までプレフィックスに結びつくとなれば、「追記のみ、書き換えなし」は性能上の助言から正しさの要件に変わります。

[^ch2-preserved-thinking]: Anthropic, "Preserved thinking", Claude API ドキュメント. https://platform.claude.com/docs/en/build-with-claude/preserved-thinking

### KV Cache は必ずしも使い捨てではない：編集可能・組み合わせ可能な「メモ」

（以下は研究の最前線からの発展的読み物で、「深水域選読」に属します。初読では飛ばして次の小節に直接進んでも、本章の以降の内容の理解には影響しません。前述の 3 つの実践的結論こそが必ず習得すべき土台です。）
...
たとえば、あるシステムプロンプトが両者を同時に使っているとします。
...
## ファイル操作
<file_operation>
- ファイルを読む前に、必ずパスが存在するかを確認する
- ファイルに書き込む前に、必ずバックアップを取る
</file_operation>

## ネットワークリクエスト
<network_request>
- タイムアウトは 30 秒に設定する
- 失敗した場合は最大 3 回まで再試行する
</network_request>
```

- **Markdown の役割**：`#`、`##` といった見出しは、人間が階層構造を一目で把握できるようにし、可読性を高めます。
- **XML の役割**：`<file_operation>`、`<network_request>` といったタグは、「このブロックはファイル操作についてのものだ」「このブロックはネットワークリクエストについてのものだ」とモデルに伝えます。意味が正確なので、モデルの処理もより正確になります。

両者が組み合わさることで、人間には読みやすく、モデルにとっても理解しやすいプロンプトになります。
...
なぜ末尾に追加すればキャッシュを壊さないのか? これはまさに前文の KV Cache のプレフィックスの性質の直接的な帰結です。因果アテンションによって、各 token の各層における隠れ状態（およびそれから計算される K、V）はその token 自身とそれより前の token だけに依存し、後続の token には依存しないので、末尾に新しい内容を追加しても、キャッシュ済みのどの token の K、V も変わりません——新たに増えたツールの schema は初出時に一度だけ計算すればよく（使い捨てのキャッシュ書き込み）、その後は絶えず成長する「プレフィックス」に組み込まれ、後続のすべてのラウンドで継続的にヒットします。ですからこれは「プリコンパイル」ではなく、「増やすだけで変えない」追加式の注入なのです。

「末尾への追加」が起こるのは、ツールが発見されたラウンドだけです。その後、schema ブロックは軌跡内の元の位置に固定され、新しいメッセージはその後ろに追加されます。ラウンドごとに最新の末尾へ移動するわけではありません。

このメカニズムのもう 1 つの制約はモデルの能力です。モデルは訓練の中で「ツール定義が対話の途中に現れる」というパターンを見ていなければなりません——これが、この能力が現時点で比較的新しいモデル（GPT-5.4+、Claude 4.5+ シリーズなど）でのみサポートされ、セルフホストのオープンソースモデルでは専門の訓練を要する理由でもあります。ツール発見の完全な議論は第 4 章「ツールが多すぎるときどうするか」の節を参照してください。
...
Agent Harness は急速に変化するため、具体的な表現は変わり得ます。安定した原則は **小さなカタログを発見可能に保ち、完全な本文をオンデマンドで読み込むこと** です。以下の 2 枚の図は、Skills の軌跡上の位置と KV Cache の変化を示します。 この設計の効果を直感的につかめるよう、次の 2 枚の図は、Skills が軌跡のどこに位置するかと KV Cache がどう変化するかを、それぞれ別の視点から追跡します。
...
イベントのサイドチャネル情報は通常、対応するイベントと一緒に追加されます。一方、タスク計画と環境状態はタスクの進行につれて絶えず更新されます。これらの動的な情報を会話履歴にどう書き込むかは KV Cache のコストに直結するため、以下では具体的なメッセージ構造と併せて論じます。
...
**第二に、思考の質を高めること——要約された知識は、生の形よりもモデルにとって使いやすいのです。** この動機は層としてより深く、また見落とされやすいものでもあります。たとえコンテキストウィンドウが十分に大きくても、生の情報をすべてコンテキストに積み上げるのが最適とは限りません。十数回の検索の生の結果はコンテキストのあちこちに散らばり、モデルは意思決定のたびに数万 token の中から関連する断片を繰り返し検索することになり、注意は分散し、重要な情報を取りこぼしやすくなります。これに対し、まず一度の LLM 呼び出しで既存の情報を「現在わかっていること：A は……、B は……、C の情報がまだ足りない」という構造化された形にまとめておけば、以降の思考はこの精錬された表現をそのまま使えます。次節では、この背後にある仕組みを説明します。
...
「黒猫と白猫はそれぞれ何匹いますか」と尋ねたとき、思考の連鎖を有効にしていないモデルが直接正しく答えるのは困難です。**検索**（「37 番の檻にいるのはどの猫か」）は注意機構の得意分野ですが、**統計的な集約**（「黒猫は全部で何匹か」）は全記録を走査してカウント状態を保持する必要があり、本質的には検索ではなく思考だからです。思考の連鎖を有効にすれば正しく数えられますが、尋ねられるたびに最初から数え直すことになります。Agent のシーンではこの種の統計が繰り返し使われることが多く、積み重なる思考コストは非常に高くつきます。これに対し、あらかじめ一度まとめて「現在の統計：黒猫 90 匹、白猫 10 匹」とコンテキストに直接書き込んでおけば、モデルはただちにこの結論を検索できます。**これが圧縮の二つ目の価値です。思考しなければ得られない結論を、直接検索できる知識に変えるのです。**
...
モデルが thinking をプレフィックスに結びつけている場合（前述の「アーキテクチャ制約としてのキャッシュ」を参照）、圧縮で支払う代価はキャッシュだけではありません。よくある「古いラウンドを要約し、直近の数ラウンドはそのまま残す」やり方は、ここで問題になります。残した数ラウンドの thinking が生成されたとき、その前にあったのは要約ではなく元の履歴です。プレフィックスはすでに変わっているので、それらはすべて無効になります。古いツール結果をその場で切り詰める場合も同じです。Anthropic が推奨するやり方は 2 つあります。会話全体を 1 つの要約メッセージに圧縮し、古い thinking は一切返さずに、モデルに要約から推論し直させる方法。もう 1 つは、コンテキストの圧縮や整理をサーバー側に任せる方法で、サーバー側によるこうした変更はプレフィックスの変更とは見なされません。
...
コンテキストエンジニアリングの本筋は、情報を明示的に管理することです。API のメッセージ構造が骨組みを定め、安定した接頭辞が KV Cache のヒット率を高め、プロンプト・Skills・ステータスバーがそれぞれルール・必要に応じた知識・現在の状態を担い、圧縮は決定・制約・失敗・出所を保持したうえで履歴の情報密度を高めます。
