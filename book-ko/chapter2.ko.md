...
그리고 이것은 기술 문제일 뿐 아니라 그 이상으로 **조직의 문제**이기도 하다. 대다수 팀에서 핵심 지식은 암묵적이다. 아키텍처 결정은 오래된 직원만 기억하고, 업무 규칙은 입에서 입으로 전해지며, 중요한 배경 정보는 개인 대화 기록 속에 잠겨 있다. 팀 자체가 정보의 블랙홀이라면 아무리 좋은 AI Agent라도 손쓸 방법이 없다.
...
이제 JSON 구조를 이해했으므로 위 단계를 Python 코드로 연결해 보겠습니다. 다음은 하나의 루프를 중심으로 만든 최소한의 에이전트 구현입니다. 이 장은 이 완전한 API 루프를 프로토콜 참조로 일부러 남겨 둔다. 다른 장에서는 Python 스타일의 골격 코드로 메커니즘을 설명한다.
...
이후에 나올 기술들은 이름이 많지만, 매 요청 직전으로 내려오면 사실 한 번의 컨텍스트 구성 결정일 뿐이다. 아래에서는 Python 스타일 의사 코드로 그 결정의 최소 골격을 남긴다. 이는 앞의 완전한 API 루프를 보완하는 것으로, 컨텍스트 배치를 강조할 뿐 메시지 역할이나 `tool_call_id` 같은 프로토콜 세부를 대체하지는 않는다.
...
시스템 프롬프트와 핵심 도구 정의는 되도록 안정적으로 유지하고, 오래된 도구 출력은 예산에 가까워질 때 한꺼번에 압축하며, 현재 상태는 궤적의 꼬리에 두어 모델이 긴 이력에서 다시 도출하지 않아도 되게 한다.
...
모델 계열마다 과거 사고 사슬을 다루는 정책은 크게 다르며, 정책 자체도 빠르게 바뀐다는 점에 유의해야 합니다. DeepSeek R1 시절의 공식 방식은 **과거 사고를 모두 제거하는 것**이었습니다. 여러 라운드의 대화에서 `content`만 돌려주고 `reasoning_content`는 돌려주지 않았습니다. R1 훈련 입력에 과거 CoT가 등장한 적이 없어 다시 넣으면 분포 밖 입력이 되어 출력을 방해할 수 있었고, 상당한 토큰도 절약할 수 있었기 때문입니다. 하지만 Agent 환경에서 이 정책은 문제가 있습니다. 중간 사고는 “왜 이 도구를 호출했는지, 어떤 가설을 배제했는지” 같은 핵심 상태를 담습니다. 이를 제거하면 모델은 라운드마다 처음부터 추론하여 실수를 반복하고 장기 계획을 잃기 쉽습니다. 그래서 DeepSeek는 V4에서 정책을 **완전히 뒤집었습니다**. 요청에 `tools` 파라미터가 있으면 두 user 메시지 사이의 모든 assistant 메시지—그 라운드에서 실제로 도구를 호출하지 않았더라도—의 `reasoning_content`를 그대로 돌려주어야 하며, 그렇지 않으면 API가 400 오류를 냅니다. `tools` 없는 순수 대화에서는 과거 사고를 여전히 무시합니다. Agent는 항상 `tools`를 함께 보내므로 이 강제 규칙을 피할 수 없습니다. Kimi K2, GLM-5 등도 같은 프로토콜을 채택했습니다. Claude도 마찬가지로 thinking block을 서명과 함께 그대로 돌려주도록 요구합니다. 최신 Claude 모델은 사용자 턴을 넘어 과거 thinking을 활용할 수도 있지만, 서명이 각 thinking block을 그것이 생성될 때의 접두부에 묶어 두기 때문에 접두부가 바뀌면 해당 block은 무효가 됩니다(이 장의 “아키텍처 제약으로서의 캐시” 절 참고). 따라서 사용하기 전에 해당 모델의 최신 문서를 확인해야 합니다. 이런 차이는 여러 턴의 대화에서는 토큰을 아끼느냐 마느냐의 문제일 뿐이지만, 절반까지 진행된 트래젝터리를 다른 벤더의 모델에 넘겨 마저 달리게 하려는 순간 실제 API 오류로 바뀝니다. 자세한 내용은 5장의 실험 5-1을 참고하세요.

**둘째, KV Cache가 접두부에 매우 민감한 이유를 알 수 있습니다.** Chat Template은 시스템 메시지와 도구 정의를 입력 앞쪽의 고정된 토큰 시퀀스로 변환합니다. 이 토큰의 Key-Value 상태는 캐시하여 요청 간에 재사용할 수 있습니다. 시스템 프롬프트에 공백 하나를 추가하는 정도라도 접두부의 토큰 하나가 바뀌면 처음 다른 토큰부터 이후의 캐시를 재사용할 수 없습니다. 그림 2-10이 보여 주는 것이 바로 이런 요청 간 접두부 재사용입니다. 아래 "KV Cache와 Prompt Cache: 두 계층의 캐시" 절의 구분에 따르면 이는 Prompt Cache 계층에서 일어나는 일이며, 재사용되는 대상은 접두부의 KV Cache입니다.
...
![그림 2-10 Prompt Cache: 접두부 KV Cache의 요청 간 재사용](images/fig2-10.svg)

**간단한 예로 KV Cache 이해하기.** 컨텍스트에 [A, B, C, D]라는 토큰 4개가 있고 모델이 다섯 번째 토큰 E를 생성하려 한다고 가정합니다. 핵심 어텐션 연산은 이렇습니다. 이 단계의 Query 벡터는 마지막으로 알려진 토큰 D의 것이며, 이를 A, B, C, D 네 토큰의 Key 벡터와 대조하여 일치 점수를 계산합니다. 내적에 대한 직관적인 설명은 실험 2-2를 참조하세요. 그런 다음 이 점수로 같은 네 토큰의 Value 벡터를 가중합하여 D 위치의 출력 표현을 만들고, 모델은 바로 이것으로 다음 토큰 E를 예측합니다. E 자신의 Q, K, V는 E가 샘플링되어 모델에 다시 입력된 뒤에야 계산됩니다.

KV Cache가 없으면 새 토큰을 생성할 때마다 접두부 전체를 처음부터 다시 순전파해야 합니다. E를 생성할 때는 A, B, C, D의 K와 V 4세트를 계산하고, 여섯 번째 토큰을 생성할 때는 E까지 포함해 5세트를 계산합니다. 접두부가 N개 토큰이 되면 N세트를 계산하므로 누적 계산량은 N²에 비례합니다.

KV Cache를 사용하면 각 토큰의 K와 V는 그 토큰이 컨텍스트에 처음 들어올 때 한 번만 계산되고 이후에는 캐시에 남습니다. E를 생성하는 단계에서 A, B, C, D의 4세트는 이미 캐시에 있으므로 그대로 꺼내 어텐션 연산을 마칠 수 있습니다. E가 샘플링되어 모델에 다시 입력된 뒤에야 E 자신의 K와 V가 계산되어 캐시에 추가되고, 캐시는 여섯 번째 토큰을 생성하기 위한 5세트가 됩니다. KV Cache는 과거 토큰의 K와 V 투영을 다시 계산하는 일을 줄여 주므로 디코딩 단계마다 전체 접두부를 재계산할 필요가 없습니다. 다만 새 토큰의 어텐션 연산은 여전히 캐시된 K와 V 전체를 순회해야 하며 계산량은 컨텍스트 길이에 선형으로 증가합니다. 긴 컨텍스트에서 디코딩이 점점 느려지고 KV Cache의 메모리와 대역폭이 추론의 병목이 되는 이유입니다.
...
**접두부를 바꾸면 비용만 늘어나는 것이 아니라 추론까지 잃을 수 있습니다.** 앞의 내용은 모두 캐시에 관한 것이었습니다. 접두부가 바뀌면 요청이 느려지고 비싸지지만 결과는 여전히 올바릅니다. Anthropic이 Claude Fable 5.1과 Claude Opus 5.5부터 도입한 “보존 사고”(preserved thinking) 메커니즘[^ch2-preserved-thinking]은 접두부 안정성에 또 하나의 의미를 더했습니다. 이 메커니즘의 원래 목적은 증류 방지입니다. 서버는 서명으로 각 thinking block을 그것이 생성될 때의 접두부, 즉 최상위 system과 tools, 그리고 그 앞의 모든 메시지에 묶어 둡니다. 이후 요청에서 이 접두부가 조금이라도 바뀌면 해당 thinking block과 그 뒤의 모든 thinking이 무효가 됩니다. 새로 가입한 계정은 기본적으로 곧바로 400 오류를 받습니다. 무효가 된 thinking을 조용히 버리도록 설정할 수도 있는데, 그러면 요청은 정상적으로 성공하지만 모델은 이번 턴에서 이전 추론을 쓰지 못하고 처음부터 다시 생각해야 합니다.

증류 방지와 캐시는 별개의 문제지만, 여기서 나오는 엔지니어링 원칙은 똑같습니다. 공식 문서도 분명하게 말합니다. thinking을 무효로 만드는 변경이 바로 캐시를 처음부터 다시 계산하게 만드는 변경이라는 것입니다. 실수하기 쉬운 패턴도 익숙한 것들입니다. 매 라운드 시스템 프롬프트를 다시 만드는 것(현재 시각이나 모드 스위치를 써넣는 등), 첫 사용자 메시지 안의 환경 정보를 매 라운드 다시 렌더링하는 것, 도중에 `tools`를 추가하거나 제거하는 것, 오래된 도구 결과를 제자리에서 잘라내는 것, 오래된 메시지에 알림을 끼워 넣었다가 다시 지우는 것입니다. 해결책도 마찬가지로 변경을 끝에 덧붙이는 것입니다. 새 지시는 대화에 덧붙이는 system 메시지로 전달하고, 환경 변화는 최신 라운드에 쓰며, 도구 추가·제거는 `tools` 배열을 고치지 않고 전용 메시지 블록으로 선언합니다. thinking block 자체도 함부로 잘라내면 안 됩니다. 앞에서부터, 뒤에서부터, 또는 전부 지울 수는 있지만, 중간의 하나를 지우고 뒤의 것을 남길 수는 없으며, 지운 것을 다시 넣을 수도 없습니다. 도중에 모델을 바꿀 때 새 모델이 원래 모델의 thinking을 읽지 못하면 API는 그 block들을 조용히 버리고, 전환 후 몇 턴 동안은 이전 추론 없이 진행하게 됩니다.

이 설계 선택에서 얻을 핵심 통찰은 **Agent 아키텍처를 설계할 때 캐시 경제성은 사후 최적화가 아니라 선행 제약**이라는 점입니다. 이 제약을 아키텍처 설계에 일찍 반영할수록 이후의 엔지니어링 비용이 줄어듭니다. 추론 사슬까지 접두부에 묶이면 “덧붙이기만 하고 고쳐 쓰지 않는다”는 원칙은 성능 권고에서 정확성 요건으로 바뀝니다.
...
(다음은 현재 연구의 선택형 심화 자료입니다. 처음 읽을 때 건너뛰고 바로 다음 소절로 넘어가도 이 장의 나머지를 이해하는 데 지장이 없습니다. 앞서 제시한 세 가지 실무 결론이 여전히 토대입니다.)
...
예를 들어 어떤 시스템 프롬프트가 둘을 함께 사용한다고 해 봅시다.
...
- **Markdown의 역할**: `#`, `##` 같은 제목은 사람이 계층 구조를 한눈에 파악하게 해 주어 가독성이 좋습니다.
- **XML의 역할**: `<file_operation>`, `<network_request>` 같은 태그는 “이 블록은 파일 작업에 관한 것”, “이 블록은 네트워크 요청에 관한 것”이라고 모델에 알려 줍니다. 의미가 정확하므로 모델도 더 정확하게 처리합니다.

둘을 함께 쓰면 사람이 읽기에도 명확하고 모델이 이해하기에도 정확합니다.
...
에이전트 하네스는 빠르게 진화하므로 구체적인 표현은 달라질 수 있습니다. 안정적인 원칙은 **작은 카탈로그를 발견 가능하게 유지하고 전체 본문을 필요할 때 로드하는 것**입니다. 다음 두 그림은 스킬이 궤적에서 놓이는 위치와 KV Cache의 변화를 보여 줍니다. 이 설계의 효과를 직관적으로 느낄 수 있도록, 다음 두 그림은 각각 다른 관점에서 Skills가 궤적에서 차지하는 위치와 KV Cache의 변화를 추적한다.
...
이벤트의 사이드 채널 정보는 보통 해당 이벤트와 함께 덧붙는다. 반면 작업 계획과 환경 상태는 작업이 진행됨에 따라 끊임없이 갱신된다. 이 동적 정보를 대화 이력에 어떻게 써 넣느냐는 KV Cache의 비용과 직결되므로, 아래에서 구체적인 메시지 구조와 함께 살펴본다.
...
**둘째, 사고의 질을 높이는 것—요약된 지식이 원래 형태보다 모델이 쓰기에 더 낫다.** 이 동기는 층위가 더 깊고 그만큼 간과되기도 쉽다. 컨텍스트 윈도가 충분히 크더라도 원시 정보를 모두 컨텍스트에 쌓아 두는 것이 최선은 아니다. 십수 회 검색의 원시 결과가 컨텍스트 곳곳에 흩어져 있으면, 모델은 결정할 때마다 수만 token 사이에서 관련 조각을 거듭 찾아야 하고, 주의가 분산되며, 핵심 정보를 놓치기 쉽다. 반면 LLM 호출 한 번으로 이미 가진 정보를 "현재까지 알려진 것: A는 …, B는 …, C에 대한 정보는 아직 부족"과 같이 구조화해 요약해 두면, 이후의 사고는 이 정제된 표현을 바로 쓸 수 있다. 다음 절에서 그 배후의 메커니즘을 설명한다.
...
"검은 고양이와 흰 고양이가 각각 몇 마리인가?"라고 물으면, 사고 사슬을 켜지 않은 모델은 곧바로 맞히기 어렵다. **찾기**("37번 우리에는 어떤 고양이가 있나?")는 어텐션의 장기이지만, **통계적 집계**("검은 고양이는 모두 몇 마리인가?")는 모든 기록을 훑으면서 계수 상태를 유지해야 하는, 본질적으로 검색이 아니라 사고이기 때문이다. 사고 사슬을 켜면 물론 제대로 셀 수 있지만, 물을 때마다 처음부터 다시 세야 한다. Agent 시나리오에서는 이런 통계가 반복해서 쓰이는 경우가 많아 누적되는 사고 비용이 매우 크다. 반면 미리 한 번 요약해 "현재 통계: 검은 고양이 90마리, 흰 고양이 10마리"를 컨텍스트에 직접 써 두면 모델은 즉시 이 결론을 검색해 낸다. **이것이 압축의 두 번째 가치다. 사고해야만 얻을 수 있는 결론을 곧바로 검색할 수 있는 지식으로 바꾸는 것이다.**
...
모델이 thinking을 접두부에 묶어 둔다면(앞의 “아키텍처 제약으로서의 캐시” 절 참고) 압축의 대가는 캐시만이 아닙니다. 흔히 쓰는 “오래된 라운드는 요약하고 최근 몇 라운드는 그대로 둔다” 방식은 여기서 문제가 됩니다. 남겨 둔 라운드의 thinking이 생성될 때 그 앞에는 요약이 아니라 원래 기록이 있었으므로 접두부가 이미 바뀌었고, 그 thinking은 모두 무효가 됩니다. 오래된 도구 결과를 제자리에서 잘라내는 것도 마찬가지입니다. Anthropic이 권장하는 방법은 두 가지입니다. 대화 전체를 하나의 요약 메시지로 압축하고 이전 thinking은 전혀 돌려주지 않아 모델이 요약에서 출발해 다시 추론하게 하거나, 컨텍스트 압축이나 정리를 서버에 맡기는 것입니다. 서버가 하는 이런 변경은 접두부 수정으로 치지 않습니다.
...
컨텍스트 엔지니어링의 큰 줄기는 정보를 명시적으로 관리하는 것이다. API 메시지 구조가 뼈대를 정의하고, 안정적인 접두부가 KV Cache 적중률을 높이며, 프롬프트와 Skills, 상태 표시줄이 각각 규칙과 필요에 따른 지식, 현재 상태를 떠맡고, 압축은 결정과 제약, 실패, 출처를 보존한 채 이력의 정보 밀도를 높인다.
