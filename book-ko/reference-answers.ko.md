...
> 적응하기 전에 진단합니다. 새 형식을 아키텍처 문서와 PRD에 대조하여 예상된 변경인지 판단합니다(실험 5-10의 발상). 버전 관리 기록을 확인하여 출처 없는 표류가 아니라 정당한 코드 커밋에 대응하는지 확인합니다. τ-bench의 log_mismatch처럼 적응하기로 해도 조용히 용인하지 말고 경고를 기록하고 이슈를 자동으로 만듭니다. 불확실하면 인간 참여형 확인으로 라우팅합니다. 원칙은 적응과 보고를 병행하여 적응이 이상 신호를 삼키지 않게 하는 것입니다.
...
**7. (★★★) 이력 재생에서 어떤 탐색 전략이 모든 이전 발견 트리에서 가장 높은 점수를 받았지만 다음 회차의 온라인 탐색에서는 퇴화했습니다. 어떤 지지 영역 편향, 무작위성, 평가 과적합이 이런 결과를 낳을 수 있을까요? 재생 세계를 어떻게 나누고 릴리스 게이트를 어떻게 설계하겠습니까?**

> 이전 발견 트리는 이전 전략이 방문한 상태와 분기만 포함하므로 미탐색 영역의 성능을 입증하지 못합니다. 온라인 과제, 도구, 환경도 달라질 수 있습니다. 난수 시드, 모델 샘플링, 도구 잡음은 순위를 바꾸며, 같은 트리에서 반복 선택하면 평가에 과적합됩니다. 과제군, 시간, 독립 트리 단위로 개발·검증·봉인된 테스트 세계를 나누고, 같은 트리와 파생 궤적은 한 집합에 둡니다. 모델·도구 버전과 계산 예산을 고정하고 여러 시드에서 기준 전략과 짝지어 비교하여 개선 폭, 불확실성, 비용, 위반율을 보고합니다. 품질·안전·비용 기준을 사전에 정합니다. 오프라인 통과 후 소규모 온라인 대조 실험에서 개선을 뒷받침하고 가드레일 퇴화가 없음을 확인한 뒤, 롤백 조건과 함께 점진적으로 배포합니다.
