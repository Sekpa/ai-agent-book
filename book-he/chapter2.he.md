...
כעת, משמבנה ה‑JSON ברור, נוכל לחבר את השלבים שלעיל ב‑Python. להלן מימוש סוכן מינימלי הבנוי סביב לולאה אחת: פרק זה שומר בכוונה על לולאת ה-API המלאה הזו כהפניה לפרוטוקול; פרקים אחרים מסבירים מנגנונים באמצעות קוד שלד בסגנון Python.
...
לטכניקות שבהמשך יש שמות רבים, אך לפני כל בקשה הן מסתכמות בהחלטה אחת של בניית ההקשר. הפסאודו-קוד בסגנון Python שלהלן משמר את השלד המינימלי של ההחלטה הזו; הוא משלים את לולאת ה-API המלאה שלמעלה בכך שהוא מדגיש את פריסת ההקשר, ואינו מחליף פרטי פרוטוקול כמו תפקידי ההודעות ו-`tool_call_id`.
...
שמרו על יציבות מרבית של פרומפט המערכת ושל הגדרות הכלים המרכזיות; דחסו פלטי כלים ישנים רק במקבצים כשמתקרבים לתקציב; והציבו את המצב הנוכחי בזנב המסלול, כדי שהמודל לא ייאלץ לגזור אותו מחדש מהיסטוריה ארוכה.
...
שימו לב שמשפחות מודלים שונות נבדלות מאוד באופן הטיפול בשרשרת מחשבה היסטורית, והאסטרטגיות עצמן מתפתחות במהירות. ההנחיה הרשמית בעידן DeepSeek R1 הייתה **להסיר את כל ההיסק ההיסטורי**: בשיחות רב‑תוריות, רק `content` מועבר בחזרה, ולא `reasoning_content` — משום ש‑CoT היסטורי מעולם לא הופיע בקלט האימון של R1, והזנתו בחזרה היא קלט מחוץ להתפלגות שעלול דווקא להפריע לפלט, והדבר גם חוסך מספר ניכר של טוקנים. אך לאסטרטגיה זו יש חסרונות בתרחישי סוכן: היסק ביניים נושא מצב קריטי כגון "מדוע נקרא כלי זה ואילו השערות נשללו"; ברגע שהוא מוסר, המודל מסיק מאפס בכל תור, ונוטה לחזור על טעויות ולאבד תוכניות ארוכות טווח. DeepSeek לפיכך **הפכה לחלוטין** את המדיניות ב‑V4: כל עוד הבקשה כוללת את הפרמטר `tools`, יש להעביר בחזרה כלשונו את ה‑`reasoning_content` של כל הודעת עוזר שבין שתי הודעות משתמש — גם כזו שבתורה לא קראה בפועל לכלי — אחרת ה‑API מחזיר שגיאת 400; בשיחה רגילה ללא `tools` ההיסק ההיסטורי עדיין נזרק. סוכן תמיד שולח `tools`, ולכן אין דרך לחמוק מהדרישה הזו — Kimi K2,‏ GLM‑5 ואחרים אימצו את אותו פרוטוקול. גם Claude דורש להעביר בחזרה את בלוק החשיבה יחד עם החתימה שלו ללא שינוי; מודלי Claude חדשים יותר יכולים אף להשתמש בחשיבה היסטורית על פני תורות משתמש, אך החתימה קושרת כל בלוק חשיבה לקידומת שבה נוצר, וכל שינוי בקידומת מבטל אותו (ראו בפרק זה את הסעיף "שמירה במטמון כאילוץ ארכיטקטוני"). עיינו בתיעוד העדכני של המודל לפני השימוש. בשיחה רב‑תורית ההבדלים האלה קובעים רק אם נחסכים טוקנים, אך ברגע שצריך למסור מסלול שלא הושלם למודל של ספק אחר כדי שיסיים אותו, הם הופכים לשגיאות ממשק ממשיות — ראו ניסוי 5‑1 בפרק 5.

**שנית, הוא מסביר מדוע KV Cache רגיש כל כך לקידומת.** ה‑Chat Template ממיר הודעות מערכת והגדרות כלים לרצף טוקנים קבוע סמוך לתחילת הקלט. מצבי המפתח‑ערך עבור טוקנים אלה ניתנים לשמירה במטמון ולשימוש חוזר בין בקשות. אם טוקן בקידומת זו משתנה — אפילו בשל רווח נוסף בהנחיית המערכת — לא ניתן עוד לעשות שימוש חוזר במטמון מהטוקן הראשון שנבדל והלאה. איור 2‑10 מציג בדיוק את השימוש החוזר הזה בקידומת בין בקשות: לפי ההבחנה שבסעיף "KV Cache ו‑Prompt Cache: שתי רמות של שמירה במטמון" להלן, הוא מתרחש ברמת ה‑Prompt Cache, ומה שנעשה בו שימוש חוזר הוא ה‑KV Cache של הקידומת.
...
ללא KV Cache, בכל פעם שנוצר טוקן חדש יש להריץ שוב מאפס את הקידומת כולה: יצירת E דורשת חישוב של 4 מערכי K ו‑V עבור A,‏ B,‏ C ו‑D, יצירת הטוקן השישי דורשת 5 מערכים, כולל E... וכשהקידומת מגיעה ל‑N טוקנים יש לחשב N מערכים, כשהחישוב המצטבר פרופורציוני ל‑N².

עם KV Cache, וקטורי ה‑K וה‑V של כל טוקן מחושבים פעם אחת בלבד, כשהטוקן נכנס להקשר לראשונה, ומאותו רגע נשארים במטמון. בצעד שמייצר את E, 4 המערכים של A,‏ B,‏ C ו‑D כבר נמצאים במטמון ודי לקרוא אותם כדי להשלים את חישוב הקשב; רק לאחר ש‑E נדגם והוזן חזרה אל המודל מחושבים ה‑K וה‑V של E עצמו ומתווספים למטמון, שגדל ל‑5 מערכים לצורך יצירת הטוקן השישי. שימו לב ש‑KV Cache חוסך את החישוב מחדש של היטלי ה‑K וה‑V עבור טוקנים היסטוריים, כך שכל צעד פענוח אינו צריך לחשב מחדש את הקידומת כולה; עם זאת, חישוב הקשב עבור כל טוקן חדש עדיין צריך לעבור על כל ערכי ה‑K וה‑V השמורים, כשהחישוב גדל לינארית עם אורך ההקשר — זו הסיבה שפענוח הקשר ארוך נעשה איטי יותר ויותר, ושהזיכרון ורוחב הפס של KV Cache הופכים לצוואר הבקבוק של האינפרנס.
...
**שינוי הקידומת לא רק מייקר — הוא עלול גם לאבד את ההיסק.** כל הסעיפים שלעיל עוסקים במטמון: אם הקידומת משתנה, הבקשה נעשית איטית ויקרה יותר, אך התוצאה עדיין נכונה. מנגנון "החשיבה השמורה" (preserved thinking)[^ch2-preserved-thinking] ש‑Anthropic הציגה החל מ‑Claude Fable 5.1 ו‑Claude Opus 5.5 מוסיף ליציבות הקידומת משמעות נוספת. מטרתו המקורית של המנגנון היא למנוע זיקוק (distillation): השרת קושר בחתימה כל בלוק חשיבה לקידומת שבה נוצר — ה‑system וה‑tools ברמה העליונה וכל ההודעות שקדמו לו. בבקשות הבאות, כל שינוי בקידומת זו מבטל את בלוק החשיבה הזה ואת כל החשיבה שאחריו. חשבונות שנרשמו לאחרונה מקבלים כברירת מחדל שגיאת 400 ישירות; אפשר גם להגדיר השלכה שקטה של חשיבה שבוטלה, והבקשה תצליח כרגיל — אך בתור הזה המודל לא יוכל להיעזר בהיסק הקודם ויאלץ לחשוב מחדש מההתחלה.

מניעת זיקוק ושמירה במטמון הן שני דברים שונים, אך המשמעת ההנדסית הנגזרת מהן זהה לחלוטין. התיעוד הרשמי אומר זאת במפורש: השינויים שמבטלים את החשיבה הם בדיוק אלה שגורמים לחישוב המטמון מחדש מאפס. גם הטעויות הנפוצות מוכרות: בנייה מחדש של הנחיית המערכת בכל תור (למשל הכנסת השעה הנוכחית או מתגי מצב), רינדור מחדש בכל תור של מידע הסביבה בהודעת המשתמש הראשונה, הוספה או הסרה של `tools` באמצע הדרך, קיצור במקום של תוצאות כלים ישנות, והכנסת תזכורת להודעה ישנה ומחיקתה לאחר מכן. גם הפתרון זהה: להוסיף את השינויים בסוף. הוראות חדשות נכנסות בהודעת system שמצורפת לשיחה, שינויי סביבה נכתבים בתור האחרון, והוספה או הסרה של כלים מוצהרות בבלוק הודעה ייעודי במקום לשנות את מערך `tools`. גם את בלוקי החשיבה עצמם אי אפשר לקצץ כרצוננו: מותר למחוק מההתחלה, מהסוף או את כולם, אך אסור למחוק בלוק מהאמצע ולהשאיר את אלה שאחריו, ואסור להחזיר בלוקים שנמחקו. בהחלפת מודל באמצע הדרך, אם המודל החדש אינו יכול לקרוא את החשיבה של המודל המקורי, ה‑API משליך את הבלוקים האלה בשקט, והתורות הראשונות שאחרי ההחלפה נותרות בלי ההיסק הקודם.

התובנה המרכזית היא ש**כלכלת המטמון אינה אופטימיזציה בדיעבד אלא אילוץ ארכיטקטוני מלכתחילה.** ככל שאילוץ זה משולב בארכיטקטורה מוקדם יותר, כך העלות ההנדסית שלאחר מכן נמוכה יותר. כאשר גם שרשרת ההיסק קשורה לקידומת, "רק להוסיף, לא לשכתב" הופך מהמלצת ביצועים לדרישת נכונות.
...
(להלן חומר מתקדם רשות ממחקר עדכני. ניתן לדלג עליו בקריאה ראשונה (ולעבור ישירות לתת‑הסעיף הבא) מבלי להשפיע על שאר הפרק; שלוש המסקנות המעשיות שלעיל הן הבסיס.)
...
### מבט קדימה: ממכניקת המטמון לעיצוב תוכן ההקשר
...
לדוגמה, הנה הנחיית מערכת שמשתמשת בשניהם יחד:
...
- **תפקיד ה‑Markdown**: כותרות כמו `#` ו‑`##` מאפשרות לאדם לקלוט את המבנה ההיררכי במבט אחד, והקריאוּת נשמרת.
- **תפקיד ה‑XML**: תגיות כמו `<file_operation>` ו‑`<network_request>` אומרות למודל ש"הבלוק הזה עוסק בפעולות על קבצים" ו"הבלוק הזה עוסק בבקשות רשת"; הסמנטיקה מדויקת, ולכן גם העיבוד של המודל מדויק יותר.

שילוב השניים מפיק הנחיה שברורה לאדם ומדויקת עבור המודל.
...
מדוע צירוף בסוף אינו שובר את המטמון? הדבר נובע ישירות מתכונת הקידומת של ה‑KV Cache שנדונה קודם: קשב סיבתי פירושו שהמצב החבוי של כל טוקן בכל שכבה (ומכאן גם ה‑K וה‑V המחושבים ממנו) תלוי רק בטוקן עצמו ובטוקנים שלפניו, ולעולם לא בטוקנים שאחריו, ולכן צירוף תוכן חדש בסוף אינו משנה את ה‑K וה‑V של אף אחד מהטוקנים השמורים במטמון — סכמת הכלי שנוספה זה עתה מחושבת פעם אחת בהופעתה הראשונה (כתיבה חד‑פעמית למטמון) ולאחר מכן מצטרפת ל"קידומת" ההולכת וגדלה, ופוגעת במטמון בכל תור עוקב. אין זו "הידור מראש" אלא הזרקה בהוספה בלבד.
...
האילוץ הנוסף של המנגנון הוא יכולת המודל: המודל חייב להיות מאומן על דפוס "הגדרות כלים המופיעות באמצע שיחה" — וזו הסיבה שרק מודלים חדשים יותר (למשל, GPT‑5.4+, סדרת Claude 4.5+) תומכים בכך כרגע, ומדוע מודלים בקוד פתוח המתארחים עצמאית זקוקים לאימון ייעודי. הדיון המלא בגילוי כלים נמצא בסעיף "מה עושים כשיש יותר מדי כלים" בפרק 4.
...
**שכבה 2 (תהליך עבודה מרכזי)**: כאשר הסוכן קובע שנדרש Skill ספציפי, סביבת זמן הריצה טוענת את `SKILL.md` המלא רק אז. יש שתי דרכים להפעיל את הטעינה הזאת: כשהמשתמש מקליד פקודת לוכסן מפורשת (כמו `/pptx`), הלקוח מיירט אותה ומרחיב אותה מקומית, כך שהמודל אינו צריך לבצע קודם קריאת כלי; וכשהמודל עצמו קורא את קטלוג המטא‑נתונים ומחליט שנדרש Skill, הוא קורא לכלי ה‑Skill הייעודי — סבב ReAct נוסף. שני המסלולים נוחתים באותו מקום: Claude Code מוסיף את גוף ה‑Skill כהודעת משתמש בנקודת ההפעלה, ובמסלול שהמודל יזם תוצאת הכלי היא רק מציין מקום המודיע שה‑Skill עולה, ואינה נושאת את הגוף[^ch2-cc-skill-inject]. סביבות זמן ריצה בלי כלי הפעלה ייעודי גורמות למודל לקרוא את `SKILL.md` בכלי קריאת קבצים כללי, ואז הגוף נכנס להקשר כתוצאת כלי. תוך שימוש ב‑Skill של PPTX‏[^ch2-4] כדוגמה, הוא מכיל את תהליך העבודה המרכזי לטיפול בקובצי PowerPoint: כיצד לחלץ טקסט באמצעות markitdown (הכלי בקוד פתוח של מיקרוסופט להמרת מסמכים ל‑Markdown), כיצד לפרוס את קובץ ה‑PPTX כדי לגשת למבנה ה‑XML הגולמי, ומוסכמות הנתיבים לקבצים מרכזיים.

[^ch2-4]: Anthropic, "PPTX Skill", 2025. https://github.com/anthropics/skills/
[^ch2-cc-skill-inject]: Claude Code Docs, [“How Claude Code uses prompt caching”](https://code.claude.com/docs/en/prompt-caching), הסעיף “Invoking skills and commands”: “Skills and commands inject their instructions as user messages at the point of invocation”. על החלוקה בין הפעלה מפורשת להפעלה ביוזמת המודל ראו Agent Skills, [“How to add skills support to your agent”](https://agentskills.io/client-implementation/adding-skills-support), הסעיף “User-explicit activation”: ה‑Harness מיירט את פקודת הלוכסן ומזריק את התוכן, כך שהמודל אינו צריך לבצע פעולת הפעלה בעצמו.
...
- **Claude Code מבחינה מושגית**: הוא חושף קטלוג קטן כהקשר זמן ריצה ומצרף את ההוראות המלאות בנקודה שבה ה‑Skill מופעל. "הנחיית מערכת" יכולה לתאר את שכבת ההוראות היציבה הלוגית, אך אין לקרוא זאת כטענה שכל לקוח משתמש בתפקיד `system` של ה‑API. איור 2‑12 מציג את המקרה שהמודל יזם, שבו המסלול כולל את הסבב המלא: tool_use של `Skill(skill: "pptx")`, tool_result שהוא מציין מקום, ולאחריו הגוף נוסף כהודעת משתמש נפרדת. אם המשתמש מקליד `/pptx` ישירות, הלקוח מרחיב את הפקודה מקומית, ולכן זוג הודעות הכלי הזה אינו מופיע ונשארת רק הודעת המשתמש האחרונה.
...
מסגרות Harness מתפתחות במהירות, ולכן ייצוגיהן הקונקרטיים עשויים להשתנות. עקרון העיצוב היציב הוא **קטלוג קטן הנשמר בר‑גילוי והגוף המלא נטען לפי דרישה**. זה מה שמאפשר ל‑Skills לשלב טעינה דינמית עם עלות הקשר מבוקרת. שני האיורים הבאים מציגים את העיצוב משתי פרספקטיבות: מיקומם של Skills במסלול והתפתחות ה‑KV Cache. כדי לחוש באופן מוחשי את השפעת התכנון הזה, שני האיורים הבאים עוקבים משתי נקודות מבט אחר מקומן של ה-Skills במסלול ואחר התפתחות ה-KV Cache.
...
מידע ערוץ-הצד של אירוע נוסף בדרך כלל יחד עם אותו אירוע; לעומת זאת, תכנון המשימה ומצב הסביבה מתעדכנים ברציפות ככל שהמשימה מתקדמת. האופן שבו המידע הדינמי הזה נכתב אל היסטוריית השיחה נוגע ישירות לעלות ה-KV Cache, ובהמשך נפרוש זאת יחד עם מבנה ההודעות הקונקרטי.
...
**שנית, שיפור איכות החשיבה — ידע מסוכם שימושי למודל יותר מצורתו הגולמית.** מניע זה עמוק יותר וגם קל יותר להתעלם ממנו. גם אם חלון ההקשר גדול דיו, ערימת כל המידע הגולמי בהקשר אינה הבחירה המיטבית: התוצאות הגולמיות של עשרות סבבי חיפוש מפוזרות בכל רחבי ההקשר, ולכן בכל החלטה המודל נאלץ לחפש שוב ושוב את הקטעים הרלוונטיים בין עשרות אלפי טוקנים, הקשב שלו מתפזר, ומידע קריטי עלול להתפספס. לעומת זאת, אם קריאת LLM אחת מסכמת תחילה את שהצטבר לכדי ייצוג מובנה — "ידוע עד כה: A הוא…, B הוא…, עדיין חסר מידע על C" — החשיבה שלאחר מכן יכולה להשתמש ישירות בייצוג המזוקק הזה. הסעיף הבא מסביר את המנגנון שמאחורי כך.

שלישית, הפגת **חרדת ההקשר (Context Anxiety)** של המודל[^ch2-7]. כאשר המודל סבור שחלון ההקשר עומד להיגמר, הוא עלול לסיים מוקדם עוד לפני שהמשימה הושלמה. דחיסה מוקדמת, בעוד חלון ההקשר רחוק מלהתמלא, עשויה לשפר את איכות ההחלטות של המודל.
...
כששואלים "כמה חתולים שחורים וכמה לבנים יש?", מודל שלא הופעלה בו שרשרת חשיבה יתקשה לענות נכון ישירות: **חיפוש** ("איזה חתול נמצא בכלוב 37?") הוא נקודת החוזק של מנגנון הקשב, ואילו **סיכום סטטיסטי** ("כמה חתולים שחורים יש בסך הכול?") מחייב לעבור על כל הרשומות ולתחזק מצב ספירה — וזו במהותה חשיבה ולא אחזור. הפעלת שרשרת חשיבה אמנם מאפשרת לספור נכון, אבל בכל שאלה צריך לספור מחדש; ובתרחישי סוכן סטטיסטיקות כאלה נדרשות שוב ושוב, כך שעלות החשיבה המצטברת גבוהה. לעומת זאת, אם מסכמים מראש פעם אחת וכותבים בהקשר ישירות "סטטיסטיקה נוכחית: 90 חתולים שחורים, 10 לבנים", המודל מאחזר את המסקנה מיד. **זהו הערך השני של הדחיסה: הפיכת מסקנות שדורשות חשיבה לידע שניתן לאחזר ישירות.**
...
אם המודל קושר את החשיבה לקידומת (ראו לעיל את הסעיף "שמירה במטמון כאילוץ ארכיטקטוני"), המחיר של דחיסה אינו רק המטמון. הגישה הנפוצה "לסכם תורות ישנות ולשמור את התורות האחרונות כלשונן" נתקלת כאן בבעיה: החשיבה של התורות שנשמרו נוצרה כשלפניה עמדה ההיסטוריה המקורית ולא הסיכום, כך שהקידומת השתנתה וכולה מתבטלת; אותו הדין לגבי קיצור במקום של תוצאות כלים ישנות. Anthropic ממליצה על אחת משתי דרכים: לדחוס את כל השיחה להודעת סיכום אחת בלי להעביר בחזרה שום חשיבה ישנה, כך שהמודל יסיק מחדש מתוך הסיכום; או להשאיר לשרת לדחוס או לנקות את ההקשר, שכן שינויים כאלה בצד השרת אינם נחשבים לשינוי הקידומת.
...
הציר המרכזי של הנדסת הקשר הוא ניהול מפורש של מידע: מבנה ההודעות של ה-API מגדיר את השלד; קידומת יציבה מעלה את שיעור הפגיעות ב-KV Cache; הפרומפט, ה-Skills ושורת המצב נושאים בהתאמה את הכללים, את הידע לפי דרישה ואת המצב הנוכחי; והדחיסה מעלה את צפיפות המידע של ההיסטוריה תוך שמירה על החלטות, אילוצים, כשלים ומקורות.
