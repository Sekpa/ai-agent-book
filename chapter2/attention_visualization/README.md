...
先准备下文中的本地模型依赖，运行默认命令得到一张热力图。确认横纵轴与词元的对应关系，再只改变一个变量：提示词、层或注意力头。先比较一对图，最后再查看跨层视图。
...
先确定横轴与纵轴分别表示哪些 token，再选中一个输出位置，观察它对哪些输入位置赋予较高权重。颜色更深表示当前可视化口径下的数值更大，并不直接表示那段输入对最终答案具有因果决定作用。比较不同层、不同头或不同输入时，应保持其他条件一致，并核对图中使用的是哪一种模型或分析方式。
